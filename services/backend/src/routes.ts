...
import { generatePairingCode, generateId, createLogger } from '@remotedev/shared-utils';
import { Device, Workspace, SyncOperation } from '@remotedev/types';
import { RemoteDevError } from '@remotedev/protocol';

const logger = createLogger('BackendRoutes');

export interface AuthenticatedRequest extends Request {
  userId?: string;
}

export function createApiRouter(db: DatabaseSync, authService: AuthService): Router {
  const router = Router();
  ...
  // -------------------------------------------------------------------------
  router.get('/devices', requireAuth, (req: AuthenticatedRequest, res: Response) => {
    const rows: any[] = db.prepare('SELECT * FROM devices WHERE user_id = ?').all(req.userId!);
  ...
  router.post('/devices/pairing-code', requireAuth, (req: AuthenticatedRequest, res: Response) => {
    ...
    db.prepare(
      'INSERT OR REPLACE INTO pairing_codes (code, laptop_device_id, user_id, expires_at, created_at) VALUES (?, ?, ?, ?, ?)'
    ).run(raw, laptopDeviceId || 'dev_laptop_001', req.userId!, expiresAt, Date.now());
  ...
  router.post('/devices/pair', requireAuth, (req: AuthenticatedRequest, res: Response) => {
    const { code, deviceName, deviceType } = req.body;
    const rawCode = String(code).replace(/\s+/g, '');

    const row: any = db.prepare('SELECT * FROM pairing_codes WHERE code = ? AND expires_at > ?').get(rawCode, Date.now());
    if (!row) {
    ...
    const androidDeviceId = generateId('dev_android');
    const now = Date.now();

    db.prepare(
      'INSERT INTO devices (id, user_id, name, device_type, platform, is_online, last_seen_at, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
    ).run(androidDeviceId, req.userId!, deviceName || 'Android Phone', deviceType || 'ANDROID_PHONE', 'android', 1, now, now);
  ...
  router.post('/devices/:id/revoke', requireAuth, (req: AuthenticatedRequest, res: Response) => {
  ...
  // Workspace Routes
  // -------------------------------------------------------------------------
  router.get('/workspaces', requireAuth, (req: AuthenticatedRequest, res: Response) => {
    const rows: any[] = db.prepare('SELECT * FROM workspaces WHERE user_id = ?').all(req.userId!);
  ...
  router.post('/workspaces', requireAuth, (req: AuthenticatedRequest, res: Response) => {
    ...
    db.prepare(
      'INSERT INTO workspaces (id, user_id, device_id, name, path, description, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
    ).run(workspaceId, req.userId!, deviceId || 'dev_laptop_001', name, path, description ?? null, now, now);
  ...
  // -------------------------------------------------------------------------
  router.post('/sync/operations', requireAuth, (req: AuthenticatedRequest, res: Response) => {
    const { workspaceId, deviceId, operations } = req.body as {
      workspaceId: string;
      deviceId: string;
    ...
    for (const op of operations) {
      const existing: any = db.prepare('SELECT operation_id FROM sync_operations WHERE operation_id = ?').get(op.operation_id);
      if (existing) {
  ...
  router.get('/sync/operations', requireAuth, (req: AuthenticatedRequest, res: Response) => {
  ...
  router.post('/sync/ack', requireAuth, (req: AuthenticatedRequest, res: Response) => {
